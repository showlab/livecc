...
from models import ModelArguments, try_optimized_architecture
...
if __name__ == "__main__":
    ...
    model = try_optimized_architecture(config.architectures[0]).from_pretrained(
    ...
    if 'Qwen2VL' in model.config.architectures[0]:
        processor = AutoProcessor.from_pretrained('Qwen/Qwen2-VL-7B-Instruct', padding_side='right') # Qwen2vl pretrained processor has some bugs. otherwise we do not need this
    else:
